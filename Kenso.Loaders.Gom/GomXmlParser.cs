...
namespace Kenso.Loaders.Gom
{
    public class GomXmlParser
    {
        ...
        public List<GomElements> GetGomElements(string pathToXmlDir)
        {
            ...
            foreach (var filePath in filePaths)
            {
                ...
                try
                {
                    ...
                    var fileElements = ParseGomXml(gomXml, fileName);
                    fileElements.Part = GetPart(fileName);
                    fileElements.Model = GetModel(fileName);
        ...
        public static GomPart GetPart(string fileName)
        {
            var splitFileName = Path.GetFileNameWithoutExtension(fileName).Split("__");
            var part = new GomPart();

            switch (splitFileName.Length)
            {
                case 3:
                    part.Number = splitFileName[1];
                    part.Name = splitFileName[2];
                    break;
                case 2:
                    part.Number = splitFileName[0];
                    part.Name = splitFileName[1];
                    break;
                default:
                    part.Number = splitFileName[0];
                    break;
        ...
        {
            var splitFileName = Path.GetFileNameWithoutExtension(fileName).Split("__");
            return splitFileName.Length == 3 ? splitFileName[0] : null;
        }
        ...
        public GomFeature ParseElement(Element element)
        {
            var feature = new GomFeature();
            feature.Name = element.Name;
            feature.Type = element.ElementType;
        ...
        public GomCharacteristic ParseElementResult(ElementResult result, string name)
        {
            var c = new GomCharacteristic()
            {
                Name = name,
                Unit = result.UnitName
            };
            var meas = new GomMeasurement();
        ...
        public List<GomCharacteristic> ParseMeasured(Result result)
        {
            var crs = new List<GomCharacteristic>();
            if (result.All != null)
            {
        ...
        public List<GomCharacteristic> ParseGeometry(Geometry geometry)
        {
            var crs = new List<GomCharacteristic>();
        ...
        public List<GomCharacteristic> ParsePosition(GeometryPosition position, string prefix = "", string postfix = "")
        {
            var crs = new List<GomCharacteristic>();

            var x = new GomCharacteristic
            {
